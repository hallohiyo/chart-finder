...
- 한국은 종목당 1회 요청이라 동시에 8개씩 보낸다(`--workers` 로 조절). 순차로 받으면
  2,600종목에 한 시간 가까이 걸리던 것이 5분 안팎으로 줄어든다. 너무 크게 잡으면
  상대 서버가 막으므로 12~16 정도가 상한이다. 실제로 몇 배 빨라지는지는
  `chartfinder speedtest -m kr` 로 직접 잴 수 있다 — 동시 요청 수를 바꿔가며
  종목/초를 재고, 서버가 동시 요청을 안 받아주면 그렇다고 알려준다.
