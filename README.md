...
한국은 네이버가 **영업활동현금흐름을 주지 않는 것으로 보여** `positive_cash_flow` 조건이
0점으로 나온다. `doctor` 가 채워진 항목과 빈 항목, 그리고 응답이 주는 원본 항목 이름을
찍어주므로 그걸로 확인한다. 정말 없다면 DART 연동이 필요하다. 미국은 6개가 모두 채워진다.
